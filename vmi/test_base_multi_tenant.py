...
class TestBaseMultiTenant(TestBaseWithSessionManager):
    ...
    def setUpClass(cls):
        ...
        from multi_tenant_manager import init_global_multi_tenant_manager, SDKFactory
    ...
    def _init_default_tenant_sdks(cls):
        ...
        try:
            ...
            logger.debug(f"多租户测试基类: 默认租户 '{cls.current_tenant_id}' 的SDK实例初始化完成")
    ...
    def setUp(self):
        ...
        if self.multi_tenant_enabled:
            # 确保当前租户的会话有效
            if not self.multi_tenant_manager.ensure_session_valid(self.current_tenant_id):
                logger.warning(f"租户 '{self.current_tenant_id}' 会话无效，尝试重新连接")
                session_mgr = self.multi_tenant_manager.get_session_manager(self.current_tenant_id)
    ...
    def run_for_tenant(self, tenant_id: str, test_func: Callable, *args, **kwargs) -> Any:
        """为指定租户运行测试函数
    ...
    def run_for_all_tenants(self, test_func: Callable, *args, **kwargs) -> Dict[str, Any]:
        """为所有启用的租户运行测试函数
        ...
        for tenant_id in enabled_tenant_ids:
            try:
                # 为每个租户运行测试
                result = self.run_for_tenant(tenant_id, test_func, tenant_id, *args, **kwargs)
    ...
    def execute_with_tenant_session_check(self, tenant_id: str, operation_func: Callable, 
                                         *args, **kwargs) -> Any:
        """在确保租户会话有效的情况下执行操作
    ...
    def assert_tenant_isolation(self, tenant1_id: str, tenant2_id: str, 
        ...
        try:
            ...
            logger.info(f"租户 '{tenant2_id}' 访问租户 '{tenant1_id}' 的数据失败，符合隔离性预期: {e}")
    ...
    def ensure_session_before_operation(self) -> bool:
        """在执行操作前确保会话有效（向后兼容）"""
        if self.multi_tenant_enabled:
            return self.multi_tenant_manager.ensure_session_valid(self.current_tenant_id)
        else:
            # 调用父类的方法
            from test_base_with_session_manager import TestBaseWithSessionManager
