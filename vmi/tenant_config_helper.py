...
def get_multi_tenant_config() -> Dict[str, Any]:
    ...
    # 检查是否启用了多租户配置
    if "multi_tenant" in base_config and base_config["multi_tenant"].get("enabled", False):
        ...
        # 设置默认租户ID
        multi_tenant_config["default_tenant"] = mt_config.get("default_tenant", "autotest")
        ...
        for tenant in mt_config.get("tenants", []):
            ...
            # 构建租户配置，使用提供的值或默认值
            tenant_config = {
                "server_url": tenant.get("server_url", default_tenant_config["server_url"]),
                "username": tenant.get("username", default_tenant_config["username"]),
                "password": tenant.get("password", default_tenant_config["password"]),
                "namespace": tenant.get("namespace", tenant_id),  # 默认使用租户ID作为namespace
...
def get_all_tenant_ids() -> List[str]:
    ...
    # 返回所有启用的租户
    return [tid for tid, tconfig in config["tenants"].items() 
...
def save_multi_tenant_config(config: Dict[str, Any], filepath: str = "test_config_multi_tenant.json") -> bool:
    """保存多租户配置到文件
