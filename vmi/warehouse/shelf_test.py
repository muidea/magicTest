...
from sdk import ShelfSDK, StatusSDK, WarehouseSDK
...
class ShelfTestCase(VMITestCase):
    """Shelf 测试用例类"""
    ...
    def setUpClass(cls):
        ...
        # 类级别的数据清理记录
        cls._class_cleanup_ids = cls.build_cleanup_registry("shelf", "warehouse")

        cls.record_initial_count(
            "_initial_shelf_count", cls._get_shelf_count, entity_name="货架"
        )
    ...
    def tearDownClass(cls):
        ...
        logger.info(
            f"测试类清理开始: 需要清理 {original_shelf_count} 个货架和 {original_warehouse_count} 个仓库"
        )
    ...
    def setUp(self):
        ...
        self.created_ids = self.build_cleanup_registry("shelf", "warehouse")
    ...
    def test_create_shelf(self):
        ...
        self.assertIsInstance(
            new_shelf["code"], (str, type(None)), "货架编码应为字符串或None"
        )
        ...
        self.assertIsInstance(
            new_shelf["used"], (int, type(None)), "使用量应为整数或None"
        )
        ...
        if "status" in new_shelf:
            # status 可能是对象或ID
            self.assertIsInstance(
                new_shelf["status"], (dict, int, type(None)), "状态应为字典、整数或None"
            )
        else:
        ...
        self.assertIsInstance(
            new_shelf["creater"], (int, type(None)), "创建者应为整数或None"
        )

        self.assertIn("createTime", new_shelf, "缺少创建时间字段")
        self.assertIsInstance(
            new_shelf["createTime"], (int, type(None)), "创建时间应为整数或None"
        )
        ...
        # 验证字段值一致性
        self.assertEqual(
            new_shelf["description"], shelf_param["description"], "描述字段不一致"
        )
        self.assertEqual(
            new_shelf["capacity"], shelf_param["capacity"], "容量字段不一致"
        )
        ...
        if isinstance(new_shelf["warehouse"], dict) and "id" in new_shelf["warehouse"]:
            self.assertEqual(
                new_shelf["warehouse"]["id"], warehouse["id"], "仓库字段不一致"
            )
        else:
            # 如果系统返回的不是对象格式，直接比较
        ...
        logger.info(
            f"成功创建货架: ID={new_shelf.get('id')}, 编码={new_shelf.get('code')}"
        )
    ...
    def test_delete_shelf(self):
        ...
        # 从清理列表中移除（因为已删除）
        if shelf_id in self.created_ids["shelf"]:
            self.created_ids["shelf"].remove(shelf_id)
    ...
    def test_create_shelf_with_long_description(self):
        ...
        logger.info(
            f"成功创建超长描述货架: ID={new_shelf.get('id')}, 描述长度={len(long_description)}"
        )
    ...
    def test_create_shelf_with_large_capacity(self):
        ...
        logger.info(
            f"成功创建大容量货架: ID={new_shelf.get('id')}, 容量={large_capacity}"
        )
    ...
    def test_create_shelf_with_zero_capacity(self):
        ...
        logger.info(
            f"成功创建小容量货架: ID={new_shelf.get('id')}, 容量={small_capacity}"
        )
    ...
    def test_update_warehouse_field(self):
        ...
        # 尝试更新仓库字段
        update_param = {"warehouse": {"id": warehouse2["id"]}}
        ...
        if updated_shelf is not None:
            ...
            if (
                isinstance(warehouse_value, dict)
                and warehouse_value.get("id") == warehouse2["id"]
            ):
                logger.warning(
                    f"系统允许更新货架的仓库字段: 从 {warehouse1['id']} 改为 {warehouse2['id']}"
                )
            elif warehouse_value == warehouse2["id"]:
                logger.warning(
                    f"系统允许更新货架的仓库字段（整数格式）: 从 {warehouse1['id']} 改为 {warehouse2['id']}"
                )
            else:
    ...
    def test_shelf_belongs_to_warehouse(self):
        ...
        if isinstance(shelf1["warehouse"], dict) and "id" in shelf1["warehouse"]:
            self.assertEqual(
                shelf1["warehouse"]["id"], warehouse1["id"], "货架1应属于仓库1"
            )
            self.assertEqual(
                shelf2["warehouse"]["id"], warehouse2["id"], "货架2应属于仓库2"
            )

            # 验证货架不属于对方的仓库
            self.assertNotEqual(
                shelf1["warehouse"]["id"], warehouse2["id"], "货架1不应属于仓库2"
            )
            self.assertNotEqual(
                shelf2["warehouse"]["id"], warehouse1["id"], "货架2不应属于仓库1"
            )
        else:
            # 如果系统返回的不是对象格式，直接比较
            ...
            # 验证货架不属于对方的仓库
            self.assertNotEqual(
                shelf1["warehouse"], warehouse2["id"], "货架1不应属于仓库2"
            )
            self.assertNotEqual(
                shelf2["warehouse"], warehouse1["id"], "货架2不应属于仓库1"
            )

        logger.info(
            f"货架属于仓库业务规则验证通过: 货架1({shelf1['id']})->仓库1({warehouse1['id']}), 货架2({shelf2['id']})->仓库2({warehouse2['id']})"
        )
