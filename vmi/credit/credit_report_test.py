...
from sdk import CreditReportSDK, PartnerSDK, StatusSDK
...
class CreditReportTestCase(VMITestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.log_suite_start("Credit Report 测试开始")
    ...
    def setUp(self):
        ...
        try:
            partner_deps = prepare_partner_dependency(
                ...
                name_prefix="CREDIT_REPORT_PARTNER",
            )
    ...
    def tearDownClass(cls):
        cls.log_suite_end("Credit Report 测试结束")
        super().tearDownClass()

    def test_create_credit_report(self):
        self.log_test_step("测试创建积分报表")
        credit_report_param = {
        ...
        self.log_test_success(f"✓ 积分报表创建成功: ID={credit_report.get('id')}")

    def test_query_credit_report(self):
        self.log_test_step("测试查询积分报表")
        credit_report_param = {
            "owner": {"id": self.test_partner["id"]},
            "credit": 1500,
            "available": 1200,
        ...
        self.log_test_success(f"✓ 积分报表查询成功: ID={queried_report.get('id')}")

    def test_update_credit_report(self):
        self.log_test_step("测试更新积分报表")
        credit_report_param = {
            "owner": {"id": self.test_partner["id"]},
            "credit": 2000,
            "available": 1800,
        ...
        if updated_report:
            ...
            self.log_test_success(f"✓ 积分报表更新成功: ID={updated_report.get('id')}")
        else:
            self.log_test_observation("⚠ 积分报表更新未返回结果")
        self.test_data.append(created_report)

    def test_delete_credit_report(self):
        self.log_test_step("测试删除积分报表")
        credit_report_param = {
            "owner": {"id": self.test_partner["id"]},
            "credit": 3000,
            "available": 2500,
        ...
        if deleted_report:
            ...
            self.log_test_success(f"✓ 积分报表删除成功: ID={deleted_report.get('id')}")
        else:
            self.log_test_observation("⚠ 积分报表删除未返回结果")

    def test_create_credit_report_with_large_credit(self):
        self.log_test_step("测试创建大积分值报表")
        credit_report_param = {
            "owner": {"id": self.test_partner["id"]},
            "credit": 999999999,
            "available": 888888888,
        ...
        self.test_data.append(credit_report)
        self.log_test_success(f"✓ 大积分值报表创建成功: 累计积分={credit_report.get('credit')}")

    def test_create_credit_report_with_zero_credit(self):
        self.log_test_step("测试创建零积分报表")
        credit_report_param = {
            "owner": {"id": self.test_partner["id"]},
            "credit": 0,
            "available": 0,
        ...
        if credit_report is None:
            self.log_test_success("✓ 系统正确拒绝创建零积分的积分报表")
        else:
            self.log_test_observation(f"⚠ 系统允许创建零积分的积分报表: ID={credit_report.get('id')}")
            self.test_data.append(credit_report)

    def test_create_credit_report_without_owner(self):
        self.log_test_step("测试创建无所属会员的积分报表")
        ...
        try:
            ...
            if credit_report is None:
                self.log_test_success("✓ 系统正确拒绝创建无所属会员的积分报表")
            else:
            ...
            if "错误代码: 4" in str(e) and "owner" in str(e):
                self.log_test_success("✓ 系统正确返回错误代码4拒绝创建无所属会员的积分报表")
            else:
    ...
    def test_query_nonexistent_credit_report(self):
        self.log_test_step("测试查询不存在的积分报表")
        non_existent_id = 999999999
        credit_report = self.credit_report_sdk.query_credit_report(non_existent_id)
        if credit_report is None:
            self.log_test_success("✓ 查询不存在的积分报表返回None，符合预期")
        else:
            self.log_test_observation(f"⚠ 查询不存在的积分报表返回: {credit_report}")

    def test_delete_nonexistent_credit_report(self):
        self.log_test_step("测试删除不存在的积分报表")
        non_existent_id = 999999999
        deleted_report = self.credit_report_sdk.delete_credit_report(non_existent_id)
        if deleted_report is None:
            self.log_test_success("✓ 删除不存在的积分报表返回None，符合预期")
        else:
            self.log_test_observation(f"⚠ 删除不存在的积分报表返回: {deleted_report}")
    ...
    def test_auto_generated_fields(self):
        ...
        self.test_data.append(credit_report)
        self.log_test_success(f"✓ 系统自动生成字段验证成功: SN={credit_report.get('sn')}")
    ...
    def test_credit_report_per_owner_limit(self):
        self.log_test_step("测试积分报表每个会员限制")
        ...
        if report2 is None:
            self.log_test_success("✓ 系统正确限制每个会员只能有一条积分报表")
        else:
            self.log_test_observation(f"⚠ 系统允许为同一会员创建多条积分报表: ID={report2.get('id')}")
