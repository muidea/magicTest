...
class CreditReportTestCase(unittest.TestCase):
    ...
    def setUp(self):
        partner_param = {'name': '测试会员-积分报表', 'telephone': '13800138000', 'status': {'id': 19}}
        try:
            self.test_partner = self.partner_sdk.create_partner(partner_param)
            if not self.test_partner:
                partners = self.partner_sdk.filter_partner({'name': '测试会员-积分报表'})
    ...
    def test_create_credit_report(self):
        print("测试创建积分报表...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 1000, 'available': 800}
        credit_report = self.credit_report_sdk.create_credit_report(credit_report_param)
        self.assertIsNotNone(credit_report, "创建积分报表失败")
        required_fields = ['id', 'sn', 'owner', 'credit', 'available', 'creater', 'createTime', 'namespace']
    ...
    def test_query_credit_report(self):
        print("测试查询积分报表...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 1500, 'available': 1200}
        ...
        queried_report = self.credit_report_sdk.query_credit_report(created_report['id'])
    ...
    def test_update_credit_report(self):
        print("测试更新积分报表...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 2000, 'available': 1800}
        created_report = self.credit_report_sdk.create_credit_report(credit_report_param)
        self.assertIsNotNone(created_report, "创建积分报表失败")
    ...
    def test_delete_credit_report(self):
        print("测试删除积分报表...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 3000, 'available': 2500}
        created_report = self.credit_report_sdk.create_credit_report(credit_report_param)
        self.assertIsNotNone(created_report, "创建积分报表失败")
        deleted_report = self.credit_report_sdk.delete_credit_report(created_report['id'])
        if deleted_report:
            self.assertEqual(deleted_report['id'], created_report['id'], "删除的积分报表ID不匹配")
    ...
    def test_create_credit_report_with_large_credit(self):
        print("测试创建大积分值报表...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 999999999, 'available': 888888888}
        credit_report = self.credit_report_sdk.create_credit_report(credit_report_param)
    ...
    def test_create_credit_report_with_zero_credit(self):
        print("测试创建零积分报表...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 0, 'available': 0}
        credit_report = self.credit_report_sdk.create_credit_report(credit_report_param)
    ...
    def test_auto_generated_fields(self):
        print("测试系统自动生成字段...")
        credit_report_param = {'owner': {'id': self.test_partner['id']}, 'credit': 500, 'available': 400}
        credit_report = self.credit_report_sdk.create_credit_report(credit_report_param)
        self.assertIsNotNone(credit_report, "创建积分报表失败")
    ...
    def test_credit_report_per_owner_limit(self):
        print("测试积分报表每个会员限制...")
        credit_report_param1 = {'owner': {'id': self.test_partner['id']}, 'credit': 1000, 'available': 800}
        report1 = self.credit_report_sdk.create_credit_report(credit_report_param1)
        self.assertIsNotNone(report1, "创建第一条积分报表失败")
        credit_report_param2 = {'owner': {'id': self.test_partner['id']}, 'credit': 2000, 'available': 1500}
