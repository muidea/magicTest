...
class CreditRewardTestCase(unittest.TestCase):
    ...
    def setUp(self):
        partner_param = {'name': '测试会员-积分消费', 'telephone': '13800138001', 'status': {'id': 19}}
        try:
            self.test_partner = self.partner_sdk.create_partner(partner_param)
            if not self.test_partner:
                partners = self.partner_sdk.filter_partner({'name': '测试会员-积分消费'})
    ...
    def test_create_credit_reward(self):
        print("测试创建积分消费记录...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 100, 'memo': '测试消费'}
        ...
        required_fields = ['id', 'sn', 'owner', 'credit', 'memo', 'creater', 'createTime', 'namespace']
    ...
    def test_query_credit_reward(self):
        print("测试查询积分消费记录...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 150, 'memo': '查询测试'}
        created_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
        self.assertIsNotNone(created_reward, "创建积分消费记录失败")
        queried_reward = self.credit_reward_sdk.query_credit_reward(created_reward['id'])
    ...
    def test_update_credit_reward(self):
        print("测试更新积分消费记录...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 200, 'memo': '更新前'}
        created_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
        self.assertIsNotNone(created_reward, "创建积分消费记录失败")
        update_param = {'memo': '更新后'}
        updated_reward = self.credit_reward_sdk.update_credit_reward(created_reward['id'], update_param)
        if updated_reward:
    ...
    def test_delete_credit_reward(self):
        print("测试删除积分消费记录...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 300, 'memo': '删除测试'}
        created_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
        self.assertIsNotNone(created_reward, "创建积分消费记录失败")
        deleted_reward = self.credit_reward_sdk.delete_credit_reward(created_reward['id'])
        if deleted_reward:
            self.assertEqual(deleted_reward['id'], created_reward['id'], "删除的积分消费记录ID不匹配")
    ...
    def test_create_credit_reward_with_large_credit(self):
        print("测试创建大积分值消费记录...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 999999, 'memo': '大额消费'}
        credit_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
    ...
    def test_auto_generated_fields(self):
        print("测试系统自动生成字段...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 50, 'memo': '自动字段测试'}
        credit_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
        self.assertIsNotNone(credit_reward, "创建积分消费记录失败")
    ...
    def test_modify_time_auto_update(self):
        print("测试修改时间自动更新...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 100, 'memo': '时间测试'}
        created_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
        self.assertIsNotNone(created_reward, "创建积分消费记录失败")
    ...
    def test_credit_reward_memo_validation(self):
        print("测试积分消费记录备注验证...")
        credit_reward_param = {'owner': {'id': self.test_partner['id']}, 'credit': 100, 'memo': '正常备注'}
        credit_reward = self.credit_reward_sdk.create_credit_reward(credit_reward_param)
        self.assertIsNotNone(credit_reward, "创建积分消费记录失败")
