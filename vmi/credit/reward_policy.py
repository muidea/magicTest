...
def main(server_url: str, namespace: str) -> bool:
    ...
    updated_reward_policy = reward_policy_sdk.update_reward_policy(
        new_reward_policy["id"], cur_reward_policy
    )
    ...
    # 再次查询验证更新
    cur_reward_policy = reward_policy_sdk.query_reward_policy(
        updated_reward_policy["id"]
    )
    if not cur_reward_policy:
        logger.error("查询积分策略失败")
        return False
    ...
    # 删除积分策略
    deleted_reward_policy = reward_policy_sdk.delete_reward_policy(
        updated_reward_policy["id"]
    )
