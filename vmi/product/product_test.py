...
class ProductTestCase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        logger.info(f"测试类清理开始: 需要清理 {original_count} 个产品: {cls._class_cleanup_ids}")
        ...
        logger.info(f"测试类清理完成: 尝试清理 {original_count} 个产品，最终产品数量: {final_product_count}")
        
        # 检查是否有数据残留（可选，根据业务需求）
        if hasattr(cls, '_initial_product_count'):
            expected_count = cls._initial_product_count
            if final_product_count > expected_count:
                logger.warning(f"可能存在数据残留: 期望数量 {expected_count}, 实际数量 {final_product_count}")
                # 尝试查找残留的产品
                cls._find_and_log_remaining_products(expected_count)
            else:
                logger.info(f"数据清理验证通过: 最终数量 {final_product_count} <= 初始数量 {expected_count}")
                ...
                if current_count > expected_count:
                    ...
                    for product in all_products:
                        if 'id' in product and 'name' in product:
                            logger.warning(f"  ID: {product['id']}, 名称: {product['name']}, 描述: {product.get('description', 'N/A')}")
        except Exception as e:
    ...
    def _cleanup_test_products(self):
        ...
        logger.info(f"测试 {self._testMethodName}: 开始清理 {len(self.created_product_ids)} 个产品: {self.created_product_ids}")
        deleted_count = 0
        failed_ids = []
        ...
        for product_id in self.created_product_ids:
            try:
                # 系统应该支持删除操作
                logger.debug(f"测试 {self._testMethodName}: 尝试删除产品 ID: {product_id}")
                result = self.product_sdk.delete_product(product_id)
                
                if result is not None:
                    deleted_count += 1
                    logger.debug(f"测试 {self._testMethodName}: 成功删除产品 {product_id}")
                    # 从类级别清理列表中移除（如果存在）
                    if (hasattr(self.__class__, '_class_cleanup_ids') and
                        product_id in self.__class__._class_cleanup_ids):
                        self.__class__._class_cleanup_ids.remove(product_id)
                        logger.debug(f"测试 {self._testMethodName}: 从类级别清理列表中移除产品 {product_id}")
                else:
                    # 删除返回None，表示删除失败
                ...
                error_msg = f"测试 {self._testMethodName}: 删除产品 {product_id} 失败: {e}"
                logger.error(error_msg)
                failed_ids.append(product_id)
                # 不抛出异常，继续尝试清理其他产品
    ...
    def _record_product_for_cleanup(self, product_id):
        ...
        if product_id is not None:
            ...
            logger.debug(f"记录产品 {product_id} 到清理列表 (测试: {self._testMethodName})")
    ...
    def test_create_product(self):
        ...
        # 验证产品信息完整性
        required_fields = ['id', 'name', 'description', 'image', 'expire', 'tags', 'status']
        for field in required_fields:
            self.assertIn(field, new_product, f"缺少字段: {field}")
        
        # 验证系统自动生成字段
        self.assertIn('creater', new_product, "缺少创建者字段")
        self.assertIsInstance(new_product['creater'], (int, type(None)), "创建者应为整数或None")
        
        self.assertIn('createTime', new_product, "缺少创建时间字段")
        self.assertIsInstance(new_product['createTime'], (int, type(None)), "创建时间应为整数或None")
        
        self.assertIn('namespace', new_product, "缺少命名空间字段")
        self.assertIsInstance(new_product['namespace'], (str, type(None)), "命名空间应为字符串或None")
    ...
    def test_update_product(self):
        ...
        updated_product = self.product_sdk.update_product(new_product['id'], update_param)
    ...
    def test_delete_product(self):
        ...
        # 删除应该成功，返回被删除的对象
        self.assertIsNotNone(deleted_product, "删除产品失败，返回None。系统应该支持删除操作")
    ...
    def test_create_product_with_many_tags(self):
        """测试创建带多个标签的产品"""
        product_param = self.mock_product_param()
        product_param['tags'] = ['tag1', 'tag2', 'tag3', 'tag4', 'tag5', 'tag6', 'tag7', 'tag8', 'tag9', 'tag10']
    ...
    def test_create_duplicate_product(self):
        ...
        if second_product is not None:
            ...
            self.assertEqual(second_product['name'], product_param['name'], "名称不匹配")
    ...
    def test_modify_time_auto_update(self):
        ...
        # 验证创建时间未改变
        self.assertEqual(updated_product.get('createTime'), initial_create_time,
                        "创建时间不应被修改")
