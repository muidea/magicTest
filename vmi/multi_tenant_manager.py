...
class MultiTenantSessionManager:
    ...
    def ensure_session_valid(self, tenant_id: str = "autotest") -> bool:
        ...
        with self.session_locks[tenant_id]:
            ...
            if (
                current_time - session_mgr.last_activity_time
                > session_mgr.session_timeout
            ):
            ...
            # 检查是否需要刷新
            if (
                current_time - session_mgr.last_refresh_time
                > session_mgr.refresh_interval
            ):
    ...
    def execute_with_session_check(
    ...
    def get_tenant_status(self, tenant_id: str) -> Dict[str, Any]:
        ...
        with self.session_locks[tenant_id]:
            return {
                ...
                "is_auto_refresh_running": session_mgr.refresh_thread is not None
                and session_mgr.refresh_thread.is_alive(),
            }
...
class SDKFactory:
    ...
    def __init__(self, multi_tenant_mgr: MultiTenantSessionManager):
        ...
        self.sdk_cache: Dict[str, Dict[str, Any]] = (
            {}
        )  # tenant_id -> {sdk_class_name: sdk_instance}
...
def init_global_multi_tenant_manager() -> Optional[MultiTenantSessionManager]:
    ...
    with _global_multi_tenant_manager_lock:
        ...
        try:
            ...
            # 检查初始化结果
            successful_tenants = [
                tid for tid, success in init_results.items() if success
            ]
            failed_tenants = [
                tid for tid, success in init_results.items() if not success
            ]
            ...
            if failed_tenants:
                logger.warning(
                    f"多租户管理器初始化部分失败，失败租户: {failed_tenants}"
                )
