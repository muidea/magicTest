...
class StockinTestCase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        if hasattr(cls, '_initial_stockin_count'):
            ...
            if final_stockin_count > expected_count:
                logger.warning(f"可能存在数据残留: 期望数量 {expected_count}, 实际数量 {final_stockin_count}")
    
    @classmethod
    def _cleanup_all_data(cls):
        """清理所有测试数据"""
        # 按依赖顺序反向清理
        cls._cleanup_entities(cls.stockin_sdk, cls._class_cleanup_ids['stockin'], '入库单')
    ...
    def _cleanup_test_entities(self):
        """清理本测试创建的实体"""
        # 按依赖顺序反向清理
        for entity_type in ['stockin', 'goods_info', 'store', 'product_info', 'shelf', 'status']:
            ...
            entity_name = {
                'stockin': '入库单', 'store': '店铺', 'goods_info': '商品信息',
                'product_info': '产品SKU', 'shelf': '货架', 'status': '状态'
            }[entity_type]
    ...
    def test_create_stockin(self):
        ...
        self.assertIsInstance(new_stockin['sn'], (str, type(None)), "入库单号应为字符串或None")
        if new_stockin['sn'] is not None:
            self.assertGreater(len(new_stockin['sn']), 0, "入库单号不应为空")
        
        self.assertIn('creater', new_stockin, "缺少创建者字段")
        self.assertIsInstance(new_stockin['creater'], (int, type(None)), "创建者应为整数或None")
        
        self.assertIn('createTime', new_stockin, "缺少创建时间字段")
        self.assertIsInstance(new_stockin['createTime'], (int, type(None)), "创建时间应为整数或None")
        
        self.assertIn('namespace', new_stockin, "缺少命名空间字段")
        self.assertIsInstance(new_stockin['namespace'], (str, type(None)), "命名空间应为字符串或None")
    ...
    def test_delete_stockin(self):
        ...
        # 删除应该成功，返回被删除的对象
        self.assertIsNotNone(deleted_stockin, "删除入库单失败，返回None。系统应该支持删除操作")
        self.assertEqual(deleted_stockin['id'], new_stockin['id'], "删除的入库单ID不匹配")
    ...
    def test_stockin_status_validation(self):
        ...
        if 'id' in new_stockin['status']:
            self.assertEqual(new_stockin['status']['id'], self.status_id, "状态ID不匹配")
        ...
        if 'goodsInfo' in new_stockin:
            ...
            if new_stockin['goodsInfo'] and len(new_stockin['goodsInfo']) > 0:
                ...
                if 'id' in goods_info_item:
                    self.assertIsInstance(goods_info_item['id'], (int, str), "商品信息ID应为整数或字符串")
        else:
    ...
    def test_modify_time_auto_update(self):
        ...
        # 验证创建时间未改变
        self.assertEqual(updated_stockin.get('createTime'), initial_create_time,
                        "创建时间不应被修改")
